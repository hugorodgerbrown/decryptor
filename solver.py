...
def find_synonyms(word: str, index: Index, limit: int = 50) -> list[Answer]:
    """Synonyms of `word`.

    A synset is small — four is the median — so there is nothing here to narrow
    down. Filtering it by a grid pattern is what the word finder is for, and
    the answer is one paste away in that tab.
...
def find_abbreviations(word: str, index: Index, limit: int = 50) -> list[Answer]:
    """What a setter may write `word` as.

    A clue word has a handful of these at most, so there is nothing to narrow
    down — the same reason find_synonyms() takes no pattern either.
